...
import { categoriaDe, culpaNossa, explicar, paraCampanha } from "@disparoy/dominio";
...
export class DisparoService {
  ...
  async planejarCampanha(job: JobCampanha): Promise<void> {
    const campanha = await carregarCampanha(this.supabase, job.campanhaId);
    if (!campanha) {
  ...
  async dispararContato(job: JobContato): Promise<void> {
    ...
    if (temCota !== true) {
      ...
      await registrarIncidente(this.supabase, this.logger, "cota_diaria_atingida", {
        canalId: canal.id,
  ...
  private async tratarSuspeitaDeCanal(
    ...
    await registrarIncidente(this.supabase, this.logger, codigo, {
      canalId: canal.id,
      canalNome: canal.nome,
      campanhaId: job.campanhaId,
      detalhe,
  ...
  private async expirarSeAgendamentoVencido(campanha: CampanhaEmExecucao): Promise<boolean> {
    ...
    await registrarAgendamentoExpirado(this.supabase, this.auditoria, this.logger, campanha, expirada);
    return true;
  }
   ...
   * fila `retencao`, uma vez por dia, não pelo `ManutencaoService` de minuto
   * em minuto.
   *
  ...
  private async finalizarSeTerminou(campanhaId: string): Promise<void> {
    ...
    const campanha = await carregarCampanha(this.supabase, campanhaId);
    await this.auditoria.registrar({
      usuarioId: null,
      usuarioNome: "Sistema",
